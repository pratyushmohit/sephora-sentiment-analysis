...
class Pipeline(BasePipeline):
    ...
    def preprocessing_pipeline(self):
        ...
        preprocessor.save_data(embedding_matrix_review_text, "embedding_matrix_review_text.npy")
        preprocessor.save_data(padded_sequences_review_text, "padded_sequences_review_text.npy")
        preprocessor.save_data(embedding_matrix_review_title, "embedding_matrix_review_title.npy")
        preprocessor.save_data(padded_sequences_review_title, "padded_sequences_review_title.npy")
        preprocessor.save_data(sequences_brand_name, "sequences_brand_name.npy")
        preprocessor.save_data(price_usd, "price_usd.npy")

        output = json.dumps({"status": "Preprocessing Pipeline successfully executed!"}, indent=4, default=str)
        return output
