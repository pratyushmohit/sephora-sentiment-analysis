...
class SentimentAnalysisModel():
    ...
    def model(self):
        ...
        input_layer_1 = Input(shape=(max_len_review_text,))
        embedding_1 = Embedding(input_dim=size_of_vocab_review_text, output_dim=300, weights=[embedding_matrix_review_text], input_length=max_len_review_text,
                                trainable=False)(input_layer_1)
        lstm = LSTM(100, return_sequences=True)(embedding_1)
        flatten_1 = Flatten()(lstm)

        input_layer_2 = Input(shape=(max_len_review_title,))
        embedding_2 = Embedding(input_dim=size_of_vocab_review_title, output_dim=50, weights=[embedding_matrix_review_title], input_length=max_len_review_title, 
                                trainable=True)(input_layer_2)
        flatten_2 = Flatten()(embedding_2)

        input_layer_3 = Input(shape=(3,))
        embedding_3 = Embedding(input_dim=project_grade_category_size_of_vocabulary, output_dim=2, input_length=1, trainable=True)(input_layer_3)
        ...
        embedding_6 = Embedding(input_dim=teacher_prefix_size_of_vocabulary, output_dim=2, input_length=1, trainable=True)(input_layer_6)
